...
class SeeRoomCategories(APIView):
    # APIView가 사용자의 요청 메소드에 따라 request를 자동으로 라우팅 해준다.
    # if, elif 쓸 필요 없다.
    # 예를 들면, get request가 오면 "get" method 호출.
    def get(self, request):
        room_categories = Category.objects.filter(kind="rooms")
        # room_categories = Category.objects.filter(kind=Category.CategoryKindChoices.ROOMS)
        serializer = CategorySerializer(room_categories, many=True)
        return Response({"ok": True, "categories": serializer.data})


class SeeExperienceCategories(APIView):
    # APIView가 사용자의 요청 메소드에 따라 request를 자동으로 라우팅 해준다.
    # if, elif 쓸 필요 없다.
    # 예를 들면, get request가 오면 "get" method 호출.
    def get(self, request):
        experience_categories = Category.objects.filter(kind="experiences")
        serializer = CategorySerializer(experience_categories, many=True)
        return Response({"ok": True, "categories": serializer.data})
